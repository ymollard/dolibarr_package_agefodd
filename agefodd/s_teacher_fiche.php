...
if ($_POST["action"] == 'confirm_delete' && $_POST["confirm"] == "yes" && $user->rights->agefodd->creer)
{
	...
	{
		$db->commit();
		Header ( "Location: s_teacher_liste.php");
		exit;
	}
	else
	{
...
if ($_POST["action"] == 'create' && $user->rights->agefodd->creer)
{
	if (! $_POST["cancel"])
	{
		...
		{
			$db->commit();
			Header ( "Location: s_teacher_fiche.php?id=".$result);
			exit;
		}
		else
		{
	...
	{
		Header ( "Location: s_teacher_fiche.php?id=".$_POST["id"]);
		exit;
	}
}
...
if ($_GET["action"] == 'create' && $user->rights->agefodd->creer)
{
	...
	print '<form name="create" action="s_teacher_fiche.php" method="post">'."\n";
	...
	if ($id)
	{
		...
		if ($result)
		{
			...
			*/
			if (isset($_GET["arch"]))
			{
				$ret=$html->form_confirm("s_teacher_fiche.php?arch=".$_GET["arch"]."&id=".$id,$langs->trans("AgfFormationArchiveChange"),$langs->trans("AgfConfirmArchiveChange"),"arch_confirm_delete");
				if ($ret == 'html') print '<br>';
			}
...
if ($_GET["action"] != 'create' && $_GET["action"] != 'edit' && $_GET["action"] != 'nfcontact')
{
	...
	if ($user->rights->agefodd->creer)
	{
		print '<a class="butActionDelete" href="s_teacher_fiche.php?action=delete&id='.$id.'">'.$langs->trans('Delete').'</a>';
	}
	else
	{
	...
	{
		print '<a class="butAction" href="s_teacher_fiche.php?action=edit&arch='.$arch.'&id='.$id.'">'.$button.'</a>';
	}
	else
	{
