...
'id' => array (
	...
	'name' => 'ref_obj','type' => 'xsd:string' 
),'ref_interne' => array (
	'name' => 'ref_interne','type' => 'xsd:string' 
),'intitule' => array (
	'name' => 'intitule','type' => 'xsd:string' 
),'duree' => array (
	'name' => 'duree','type' => 'xsd:string' 
),'public' => array (
	'name' => 'public','type' => 'xsd:string' 
),'methode' => array (
	'name' => 'methode','type' => 'xsd:string' 
),'prerequis' => array (
	'name' => 'prerequis','type' => 'xsd:string' 
),'but' => array (
	'name' => 'but','type' => 'xsd:string' 
),'programme' => array (
	'name' => 'programme','type' => 'xsd:string' 
),'note1' => array (
	'name' => 'note1','type' => 'xsd:string' 
),'note2' => array (
	'name' => 'note2','type' => 'xsd:string' 
),'archive' => array (
	'name' => 'archive','type' => 'xsd:string' 
),'note_private' => array (
	'name' => 'note_private','type' => 'xsd:string' 
),'note_public' => array (
	'name' => 'note_public','type' => 'xsd:string' 
),'fk_product' => array (
...
$server->register ( 'getAgefodd', 
		...
		// Exit values
		array (
			'result' => 'tns:result','agefodd' => 'tns:agefodd' 
...
$server->register ( 'createAgefodd', 
		// Entry values
		array (
			'authentication' => 'tns:authentication','agefodd' => 'tns:agefodd' 
		), 
		// Exit values
		array (
 ...
 * Get Agefodd
 *
 * @param array $authentication authentication information
 * @param int $id object
 * @param string $ref object
 * @param ref_ext $ref_ext of object
 * @return mixed
 */
	...
	if (! $error) {
		...
		if ($fuser->rights->agefodd->read) {
			...
			if ($result > 0) {
				...
				'id' => $agefodd->id,'entity' => $agefodd->entity,'ref' => $agefodd->ref,'ref_obj' => $agefodd->ref_obj,'ref_interne' => $agefodd->ref_interne,'intitule' => $agefodd->intitule,'duree' => $agefodd->duree,'public' => $agefodd->public,'methode' => $agefodd->methode,'prerequis' => $agefodd->prerequis,'but' => $agefodd->but,'programme' => $agefodd->programme,'note1' => $agefodd->note1,'note2' => $agefodd->note2,'archive' => $agefodd->archive,'note_private' => $agefodd->note_private,'note_public' => $agefodd->note_public,'fk_product' => $agefodd->fk_product,'nb_subscribe_min' => $agefodd->nb_subscribe_min,'fk_formation_catalogue' => $agefodd->fk_formation_catalogue,'priorite' => $agefodd->priorite,'lines' => $agefodd->lines,'canvas' => $agefodd->canvas,'lastname' => $agefodd->lastname,'firstname' => $agefodd->firstname,'name' => $agefodd->name,'nom' => $agefodd->nom,'civility_id' => $agefodd->civility_id,'array_options' => $agefodd->array_options,'linkedObjectsIds' => $agefodd->linkedObjectsIds,'linkedObjects' => $agefodd->linkedObjects 
