			...
			if (empty($conf->global->MAIN_EXTRAFIELDS_DISABLED)) // For avoid conflicts if trigger used
{
	...
	public function fetch_all($sortorder, $sortfield, $limit, $offset, $filter = array(), $user = 0, $array_options_keys=array()) {
		...
		$sql .= " GROUP BY s.rowid,soemployer.nom,dictstatusba.intitule, s.fk_soc, s.fk_session_place, s.type_session, s.dated, s.datef,  s.status, dictstatus.intitule , dictstatus.code,  s.date_res_trainer, s.color, s.force_nb_stagiaire, s.nb_stagiaire,s.notes,";
...
*/
	 ...
	 * @return int <0 if KO, >0 if OK
	 */
	public function createInvoice($user, $socid, $from_financial_id = 0, $amount = 0, $from_financial_elementtype = 'propal') {
	 ...
	 * @param string $socid
	 * @param string $obj_agefodd_convention
