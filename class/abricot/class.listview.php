		...
		*/
				...
				foreach ($THeader as $field => $dummy) {
					...
					if (!in_array($field, $TParam['hide'])) {
						...
						if (isset($TParam['type'][$field]) && !isset($TParam['eval'][$field])) {
							...
							if ($TParam['type'][$field] == 'hour') {
								$row[$field] = date('H:i', strtotime($row[$field]));
							}
							if ($TParam['type'][$field] == 'money') {
								$row[$field] = '<div align="right">' . price($row[$field], 0, '', 1, -1, 2) . '</div>';
							}
							if ($TParam['type'][$field] == 'number') {
								$row[$field] = '<div align="right">' . price($row[$field]) . '</div>';
							}
							if ($TParam['type'][$field] == 'integer') {
								$row[$field] = '<div align="right">' . ((int)$row[$field]) . '</div>';
							}
						}
		...
		private function parse_sql(&$THeader, &$TField, &$TParam, $sql)
		{
			...
			if ($res !== false) {
				...
				while ($currentLine = $this->db->fetch_object($res)) {//$THeader, &$TField, &$TParam, &$TFieldInView, $currentLine
