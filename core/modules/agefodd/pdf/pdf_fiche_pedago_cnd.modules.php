...
class pdf_fiche_pedago_cnd extends ModelePDFAgefodd {
		...
		if (file_exists($dir)) {
			...
			if ($result) {
				...
				*/
				...
				if (!empty($agf_session->id)) {
					require_once '../class/agefodd_session_calendrier.class.php';
					$calendrier = new Agefodd_sesscalendar($this->db);
					$result = $calendrier->fetch_all($agf_session->id);
					if ($result < 0) {
					...
					foreach($calendrier->lines as $line) {
						$array_day[$line->date_session]=$line->date_session;
					}
					$jour = count($array_day);
				} else {
					$jour = $agf->array_options['nbdays'];
				}
				...
				//else {
					$this->str = $jour . ' ' . $outputlangs->transnoentities('AgfPDFFichePeda3');
					if (ceil($jour) > 1)
						$this->str .= 's';
				//}
				$this->pdf->SetXY($posX, $posY);
