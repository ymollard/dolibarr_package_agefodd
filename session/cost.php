	...
	if ($result < 0) {
		...
		if (count($session_invoice->lines)>0) {
			...
			foreach($session_invoice->lines as $line) {
				...
				$totalttc+=$suplier_invoice->total_ttc;
			}
		}
		$agf->cost_trainer=$totalttc;
...
if ($action == 'unlink')
{
	...
	$ret=$form->form_confirm($_SERVER['PHP_SELF'].'?socid='.$socid.'&id='.$id.'&idelement='.$idelement,$langs->trans("AgfConfirmUnlink"),
...
if ($action == 'link')
{
	...
	$form_question=array();
	$form_question[]=array('label'=> $langs->trans("AgfFactureSelectInvoice"),
	'type'=> 'select','values'=>$invoice_array,
	'name'=>'invoiceselected');

	
	$ret=$form->form_confirm($_SERVER['PHP_SELF'].'?socid='.$socid.'&id='.$id.'&idelement='.$idelement,$langs->trans("AgfFactureSelectInvoice"),
			'',"link_confirm",$form_question,'',1);
	if ($ret == 'html') print '<br>';
}
	...
	if (! empty ( $line->socpeopleid )) {
		...
		if ($contact_static->thirdparty->fournisseur == 1) {
			...
			if ($action=='createinvoice_supplier' && $opsid==$line->opsid) {
				...
				print $langs->trans('PriceUHT').'<input type="text" class="flat" size="4" name="pricetrainer" value="' . GETPOST ( 'pricetrainer' . $line->opsid ) . '">' . $langs->getCurrencySymbol ( $conf->currency );
				print '</td>';
				...
				print '<td  align="left" style="padding-left:10px">';
				print $langs->trans('Qty').'<input type="text" class="flat" size="2" name="qtytrainer" value="' . GETPOST ( 'qtytrainer' . $line->opsid ) . '">';
				print '</td>';
				...
				print '<a href="' . dol_buildpath ( '/agefodd/session/cost.php', 1 ) . '?action=link&id=' . $id . '&opsid=' . $line->opsid.'&socid=' . $contact_static->thirdparty->id . '" alt="' . $legende . '" title="' . $legende . '">';
