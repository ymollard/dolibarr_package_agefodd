...
if (! empty ( $id )) {
	...
	if ($action == 'edit') {
		...
		if (! empty ( $_POST ["modstagid"] ) && $action == 'edit') {
			print '<script type="text/javascript">
					jQuery(document).ready(function () {
						jQuery(function() {
							var documentBody = (($.browser.chrome)||($.browser.safari)) ? document.body : document.documentElement;
		    				 $(documentBody).animate({scrollTop: $("#modstagid' . $_POST ["modstagid"] . '").offset().top}, 500,\'easeInOutCubic\');
						});
					});
					</script> ';
		}
		...
		if ($nbstag > 0) {
			...
			for($i = 0; $i < $nbstag; $i ++) {
				...
				if ($stagiaires->lines [$i]->id == $_POST ["modstagid"] && $_POST ["stag_remove_x"])
					...
					print '<tr>';
				print '<a name="modstagid' . $stagiaires->lines [$i]->id . '" id="modstagid' . $stagiaires->lines [$i]->id . '"/>';
				...
				if ($stagiaires->lines [$i]->id == $_POST ["modstagid"] && ! $_POST ["stag_remove_x"]) {
					...
					*/
					if ($agf->type_session == 1 && ! $_POST ['cancel'] && $show_subrogation && ! empty ( $conf->global->AGF_MANAGE_OPCA )) {
						...
						$events = array ();
						$events [] = array (
							'method' => 'getContacts','url' => dol_buildpath ( '/core/ajax/contacts.php', 1 ),'htmlname' => 'fksocpeopleOPCA','params' => array (
						...
						$socbiller->fetch ( $stagiaires->lines [$i]->socid );
						// var_dump($socbiller);
					...
					if (! $show_subrogation && ! empty ( $conf->global->AGF_MANAGE_OPCA ) && ($agf->type_session == 1))
