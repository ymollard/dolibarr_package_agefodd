...
CREATE OR REPLACE FUNCTION TIME_TO_SEC(t int) RETURNS timestamp AS $$ DECLARE hs INTEGER; ms INTEGER; s INTEGER; BEGIN SELECT (EXTRACT( HOUR FROM  t::time) * 60*60) INTO hs; SELECT (EXTRACT (MINUTES FROM t::time) * 60) INTO ms; SELECT (EXTRACT (SECONDS from t::time)) INTO s; SELECT (hs + ms + s) INTO s; RETURN s; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION TIME_TO_HOUR(t int) RETURNS timestamp AS $$ DECLARE h INTEGER; BEGIN SELECT (EXTRACT( HOUR FROM  t::time)) INTO h; RETURN h; END; $$ LANGUAGE plpgsql;
