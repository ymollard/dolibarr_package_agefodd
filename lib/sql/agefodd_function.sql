...
CREATE OR REPLACE FUNCTION TIME_TO_SEC(t int) RETURNS INTEGER AS $$ DECLARE hs INTEGER; ms INTEGER; s INTEGER; BEGIN SELECT (EXTRACT( HOUR FROM  t::time) * 60*60) INTO hs; SELECT (EXTRACT (MINUTES FROM t::time) * 60) INTO ms; SELECT (EXTRACT (SECONDS from t::time)) INTO s; SELECT (hs + ms + s) INTO s; RETURN s; END; $$ LANGUAGE plpgsql;
DROP FUNCTION time_to_hour(integer);
CREATE OR REPLACE FUNCTION TIME_TO_HOUR(t int) RETURNS INTEGER AS $$ DECLARE h INTEGER; BEGIN SELECT (EXTRACT( HOUR FROM  t::time)) INTO h; RETURN h; END; $$ LANGUAGE plpgsql;
